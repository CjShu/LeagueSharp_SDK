namespace Flowers_Series.Plugings
{
    ...
    public static class Akali
    {
        ...
        internal static Obj_AI_Hero Me => Program.Me;
        ...
        private static void Combo()
        {
            ...
            if (CheckTarget(target))
            {
                ...
                if (!Menu["Akali_Combo"]["R"] || !R.IsReady()) return;
        ...
        {
            if (!(Me.ManaPercent >= Menu["Akali_Harass"]["Mana"].GetValue<MenuSlider>().Value)) return;
        ...
        {
            if (!(Me.ManaPercent >= Menu["Akali_LaneClear"]["Mana"].GetValue<MenuSlider>().Value)) return;
            ...
            if (GetMinions(Me.Position, Q.Range).Count <= 0 || !Menu["Akali_LaneClear"]["Q"] || !Q.IsReady()) return;
        ...
        {
            if (!(Me.ManaPercent >= Menu["Akali_JungleClear"]["Mana"].GetValue<MenuSlider>().Value)) return;
        ...
        {
            if ((!InLastHit || !Menu["Akali_LastHit"]["Q"] ||
                 !(Me.ManaPercent >= Menu["Akali_LastHit"]["Mana"].GetValue<MenuSlider>().Value)) &&
                (!InHarass || !Menu["Akali_Harass"]["QLastHit"] ||
                 !(Me.ManaPercent >= Menu["Akali_Harass"]["Mana"].GetValue<MenuSlider>().Value))) return;
        ...
        private static void KillSteal()
        {
            foreach (var target in GetEnemies(R.Range))
            {
                ...
                if (!Menu["Akali_KillSteal"]["R"] || !(target.Health < R.GetDamage(target)) ||
                    ...
                    !target.IsValidTarget(R.Range)) continue;
        ...
        private static void OnDraw(EventArgs Args)
        {
            ...
            {
                foreach (var target in ObjectManager.Get<Obj_AI_Hero>().Where(e => e.IsValidTarget() && e.IsValid && !e.IsDead && !e.IsZombie))
                {
            ...
            if (Menu["Akali_Draw"]["DrawR"])
            {
                ...
                text = Menu["Akali_Combo"]["RTower"].GetValue<MenuKeyBind>().Active ? "On" : "Off";
        ...
        private static int RCount => Me.HasBuff("AkaliShadowDance") ? Me.GetBuffCount("AkaliShadowDance") : 0;
    }
}
