    ...
    public static class Ashe
    {
        ...
        public static void Init()
        {
            ...
            var ClearMenu = Menu.Add(new Menu("Ashe_Clear", "Clear"));
            {
                ...
                ClearMenu.Add(new MenuSlider("LCWMana", "If Player ManaPercent >= %", 50));
        ...
        private static void OnInterruptableTarget(object sender, Events.InterruptableTargetEventArgs Args)
        {
            if (!Menu["Ashe_RSettings"]["Interrupt"] || !R.IsReady() || !Args.Sender.IsEnemy ||
                Args.DangerLevel < LeagueSharp.Data.Enumerations.DangerLevel.High || !Args.Sender.IsValidTarget(R.Range))
                return;
        ...
        private static void OnUpdate(EventArgs args)
        {
            ...
            if (!Menu["Ashe_ESettings"]["AutoE"] || !E.IsReady()) return;
        ...
        {
            if (!Menu["Ashe_RSettings"]["R"].GetValue<MenuKeyBind>().Active || !R.IsReady()) return;
        ...
        private static void ComboLogic()
        {
            ...
            if (!Menu["Ashe_Combo"]["R"] || !R.IsReady()) return;
        ...
        {
            if (!Menu["Ashe_Harass"]["W"] ||
                !(Menu["Ashe_Harass"]["WMana"].GetValue<MenuSlider>().Value <= Me.ManaPercent) || !W.IsReady() ||
                Me.HasBuff("AsheQAttack")) return;
        ...
        private static void LaneLogic()
        {
            ...
            if (!Menu["Ashe_Clear"]["LCW"] || !W.IsReady() ||
                !(Menu["Ashe_Clear"]["LCWMana"].GetValue<MenuSlider>().Value <= Me.ManaPercent)) return;
        ...
        private static void KillStealLogic()
        {
            if (Menu["Ashe_KillSteal"]["KSW"] && W.IsReady())
            {
                ...
                {
                    if (!target.IsValid || !(target.Health < W.GetDamage(target))) continue;
            ...
            if (!Menu["Ashe_KillSteal"]["KSR"] || !R.IsReady()) return;
            ...
            {
                if (!(target.DistanceToPlayer() > 800) || !target.IsValid || !(target.Health < R.GetDamage(target)) ||
                    target.HasBuffOfType(BuffType.SpellShield)) continue;
        ...
        private static void OnAction(object sender, OrbwalkingActionArgs Args)
        {
                ...
                if (Args.Target is Obj_AI_Minion && InClear)
                {
                    ...
                    foreach (var mob in Mobs)
                    {
                        ...
                        if (!Menu["Ashe_Clear"]["JCQ"].GetValue<MenuSliderButton>().BValue ||
                            !mob.IsValidTarget(GetAttackRange(Me)) || !Q.IsReady() || !Me.HasBuff("asheqcastready") ||
                            !(mob.Health > Me.GetAutoAttackDamage(mob)*2)) continue;

                        if (!(Menu["Ashe_Clear"]["JCQ"].GetValue<MenuSliderButton>().SValue < Me.ManaPercent)) continue;
            ...
            if (Args.Type != OrbwalkingType.AfterAttack || Me.HasBuff("AsheQAttack")) return;
            ...
            if (InCombo)
            {
                ...
                if (CheckTarget(target))
                {
                    ...
                    {
                        if (Menu["Ashe_Combo"]["SaveMana"] && Me.Mana < R.Instance.ManaCost + W.Instance.ManaCost + Q.Instance.ManaCost)
                        {
                            return;
                        }
