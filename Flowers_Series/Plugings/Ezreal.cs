    ...
    public static class Ezreal
    {
        ...
        public static void Init()
        {
            ...
            var RMenu = Menu.Add(new Menu("Ezreal_RMenu", "R Settings"));
            {
                ...
                RMenu.Add(new MenuBool("Steal", "Auto Steal Jungle"));
        ...
        private static void HarassLogic()
        {
            if (Menu["Ezreal_Harass"]["Q"] && Q.IsReady())
            {
                ...
                {
                    if (!Menu["Ezreal_QList"][target.ChampionName.ToLower()] &&
        ...
        private static void AutoRLogic()
        {
            if (R.IsReady() && Menu["Ezreal_RMenu"]["Auto"])
            {
                ...
                foreach (var t in GetEnemies(R.Range))
                {
                    ...
                    var CheckTargetHealth = t.Health + t.MagicalShield + t.HPRegenRate * 2 < RDMG;
        ...
        private static void StealLogic()
        {
            if (R.IsReady() && Menu["Ezreal_RMenu"]["Steal"])
            {
                ...
                {
                    if (mob != null && (mob.SkinName == "SRU_Dragon" || mob.SkinName == "SRU_Baron" || mob.SkinName == "SRU_Red" || mob.SkinName == "SRU_Blue") && Me.Distance(mob) >= Q.Range)
                    {
            ...
            if (Args.Type == OrbwalkingType.AfterAttack)
            {
                ...
                if (hero != null)
                {
                    ...
                    if (InHarass && Menu["Ezreal_Harass"]["Q"] && !Me.IsUnderEnemyTurret())
                    {
                        ...
                        if (target.IsValidTarget(Q.Range) && Q.IsReady() && Menu["Ezreal_QList"][hero.ChampionName.ToLower()] && Me.ManaPercent >= Menu["Ezreal_Harass"]["QMana"].GetValue<MenuSlider>().Value)
                        {
                            Q.CastTo(target);
                        }
                    }
        ...
        private static void OnDraw(EventArgs Args)
        {
            ...
            if (Menu["Ezreal_Draw"]["AutoQ"])
            {
                ...
                text = Menu["Ezreal_Auto"]["Q"].GetValue<MenuKeyBind>().Active ? "On" : "Off";
            ...
            if (Menu["Ezreal_Draw"]["AutoW"])
            {
                ...
                text = Menu["Ezreal_Auto"]["W"].GetValue<MenuKeyBind>().Active ? "On" : "Off";
