    ...
    public class Hecarim
    {
        ...
        private static void OnDraw(EventArgs Args)
        {
            ...
            if (Menu["Hecarim_Draw"]["Auto"])
            {
                ...
                text = Menu["Hecarim_Harass"]["Auto"].GetValue<MenuKeyBind>().Active ? "On" : "Off";
        ...
        private static void Lane()
        {
            ...
            if (Minions.Count > 0)
            {
                if (Me.ManaPercent >= Menu["Hecarim_LaneClear"]["Mana"].GetValue<MenuSlider>().Value)
                {
                    ...
                    {
                        if (Minions.Count(x => x.IsValidTarget(Q.Range)) >= Menu["Hecarim_LaneClear"]["Q"].GetValue<MenuSliderButton>().SValue)
                        {
                            Q.Cast();
                        }
                    }
                    ...
                    {
                        if (Minions.Count(x => x.IsValidTarget(W.Range)) >= Menu["Hecarim_LaneClear"]["W"].GetValue<MenuSliderButton>().SValue)
                        {
                            W.Cast();
                        }
                    }
        ...
        private static void KillSteal()
        {
            ...
            {
                var wt = GameObjects.EnemyHeroes.FirstOrDefault(x => x.IsValidTarget(W.Range) && x.Health < W.GetDamage(x));
            ...
            {
                var rt = GameObjects.EnemyHeroes.FirstOrDefault(x => x.IsValidTarget(R.Range) && x.Health < R.GetDamage(x) &&
                Menu["Hecarim_KillSteal"][x.ChampionName.ToLower()] &&
                x.DistanceToPlayer() >= Menu["Hecarim_KillSteal"]["R"].GetValue<MenuSliderButton>().SValue);
