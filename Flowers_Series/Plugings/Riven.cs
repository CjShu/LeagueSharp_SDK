    ...
    public class Riven
    {
        ...
        private static void CastSpellLogic(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs Args)
        {
            ...
            if (InCombo)
            {
                ...
                target = Variables.TargetSelector.GetSelectedTarget() != null ? GetTarget(E.Range + W.Range) : Variables.TargetSelector.GetSelectedTarget();
        ...
        private static void OnDraw(EventArgs Args)
        {
            ...
            if (Menu["Riven_Draw"]["DrawR1"])
            {
                ...
                text = Menu["Riven_Combo"]["R"].GetValue<MenuKeyBind>().Active ? "Enable" : "Off";
        ...
        {
            if (GetMinions(Me.Position, W.Range).Any())
            {
        ...
        {
            if (GetMobs(Me.Position, W.Range).Any())
            {
        ...
        private static void RLogic()
        {
            ...
            if (CheckTarget(target))
            {
                    ...
                    (!IsBurstActive || (IsBurstActive && Variables.TargetSelector.GetSelectedTarget() == null)))
                {
                    ...
                    {
                        if (target.HealthPercent < 25 && target.Health > R.GetDamage(target) + Me.GetAutoAttackDamage(target) * 2)
                        {
                            CastR2();
                        }
        ...
        private static void FleeLogic()
        {
            ...
            {
                var enemy = GameObjects.EnemyHeroes.Any(x => x.IsValidTarget(W.Range) && !x.IsDead);
