    ...
    public class Ryze
    {
        ...
        private static void Harass()
        {
            if (Me.ManaPercent >= Menu["Ryze_Harass"]["Mana"].GetValue<MenuSlider>().Value)
            {
                ...
                var min = minions.FirstOrDefault(x => x.IsValidTarget(E.Range) && x.Position.Distance(target.Position) <= 300);
        ...
        private static void Lane()
        {
            if (Me.ManaPercent >= Menu["Ryze_LaneClear"]["Mana"].GetValue<MenuSlider>().Value)
            {
                ...
                if (minions.Count > 0)
                {
                    ...
                    if (minion != null && (Menu["Ryze_LaneClear"]["Q"] && Q.IsReady() && !minion.HasBuff("RyzeE")))
                    {
        ...
        private static void LastHit()
        {
            if (Me.ManaPercent >= Menu["Ryze_LastHit"]["Mana"].GetValue<MenuSlider>().Value)
            {
                ...
                if (Minions.Count > 0)
                {
                    ...
                    if (min != null && (Menu["Ryze_LastHit"]["E"] && E.IsReady() && min.IsValidTarget(E.Range) && min.Health < E.GetDamage(min)))
                    {
                        E.Cast(min);
                        return;
                    }

                    if (min != null && (Menu["Ryze_LastHit"]["W"] && W.IsReady() && min.IsValidTarget(W.Range) && min.Health < W.GetDamage(min)))
                    {
                        W.Cast(min);
                        return;
                    }

                    if (min != null && (Menu["Ryze_LastHit"]["Q"] && Q.IsReady() && !Me.HasBuff("ryzeqiconfullcharge") && min.IsValidTarget(Q.Range) && min.Health < Q.GetDamage(min)))
                    {
                        Q.Cast(min);
                    }
                }
        ...
        private static void OnGapCloser(object obj, Events.GapCloserEventArgs Args)
        {
            if (Menu["Ryze_Misc"]["WGap"] && W.IsReady() && Args.Sender.IsEnemy && 
                Args.Sender != null && Args.Sender.IsValidTarget(250))
            {
        ...
        private static void CastSpell(Obj_AI_Hero target, bool useQ, bool useW, bool useE)
        {
            if (CheckTarget(target))
            {
