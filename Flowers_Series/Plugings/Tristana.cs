    ...
    public static class Tristana
    {
        ...
        public static void Init()
        {
            ...
            var EListMenu = Menu.Add(new Menu("Tristana_EList", "E BlackList"));
            {
                ...
                {
                    GameObjects.EnemyHeroes.ForEach(i => EListMenu.Add(new MenuBool(i.ChampionName.ToLower(), i.ChampionName)));
                }
            }
        ...
        private static void OnAction(object obj, OrbwalkingActionArgs Args)
        {
            if (Args.Type == OrbwalkingType.BeforeAttack)
            {
                ...
                {
                    if (InCombo && Args.Target is Obj_AI_Hero)
                    {
            ...
            if (Args.Type == OrbwalkingType.AfterAttack && InClear && (Args.Target is Obj_AI_Turret || Args.Target.Type == GameObjectType.obj_AI_Turret))
            {
                ...
                if (t != null && (Q.IsReady() && Menu["Tristana_LaneClear"]["Q"] && Me.CountEnemyHeroesInRange(1000) < 1 && (t.HasBuff("TristanaEChargeSound") || t.HasBuff("TristanaECharge"))))
                {
                    Q.Cast();
                }
            }
        ...
        {
            return Me.GetSpellDamage(target, SpellSlot.E) * (target.GetBuffCount("TristanaECharge") * 0.30) + Me.GetSpellDamage(target, SpellSlot.E);
        }
    }
}
