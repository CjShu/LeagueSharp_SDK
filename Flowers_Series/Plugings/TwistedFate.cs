namespace Flowers_Series.Plugings
{
    ...
    public class TwistedFate
    {
        ...
        public static void Init()
        {
            ...
            {
                HumanizerMenu.Add(new MenuBool("EnableHumanizer", "Enable"));
            ...
            {
                MiscMenu.Add(new MenuBool("ADMode", "AD Mode"));
        ...
        private static void ComboLogic()
        {
            ...
            if (CheckTarget(target))
            {
                ...
                {
                    if (Menu["TwistedFate_Combo"]["SaveMana"] && Me.Mana > Q.Instance.ManaCost + W.Instance.ManaCost)
                    {
                ...
                if (W.IsReady() && target.IsValidTarget(W.Range))
                {
                    ...
                    else if (Me.Mana + W.Instance.ManaCost <= Q.Instance.ManaCost + W.Instance.ManaCost && Menu["TwistedFate_Combo"]["WMP"])
                    {
                        Card.ToSelect(Cards.Blue);
                    }
        ...
        private static void JungleClearLogic()
        {
            if (Me.ManaPercent >= Menu["TwistedFate_JungleClear"]["Mana"].GetValue<MenuSlider>().Value)
            {
                ...
                if (mobs.Count > 0)
                {
                    ...
                    {
                        if (mob != null && mob.Health >= Me.GetAutoAttackDamage(mob))
                        {
                            Q.Cast(mob);
                        }
                    }
                    ...
                    if (Menu["TwistedFate_JungleClear"]["W"] && W.IsReady() && mob.IsValidTarget(W.Range))
                    {
                        ...
                        else if (mob != null && mob.Health < W.GetDamage(mob))
                        {
                            Card.ToSelect(Cards.Blue);

                            if (Card.Status == SelectStatus.Selected)
                            {
                                Me.IssueOrder(GameObjectOrder.AttackUnit, mob);
                            }
                        }
                    }
                }
        ...
        private static void OnAction(object sender, OrbwalkingActionArgs Args)
        {
            ...
            {
                if (InCombo && Args.Target is Obj_AI_Hero && Menu["TwistedFate_Misc"]["ComboDisableAA"] && !Menu["TwistedFate_Misc"]["ADMode"])
                {
        ...
        private static void OnDraw(EventArgs Args)
        {
            ...
            if (DrawW)
            {
                var FlowersStyle = System.Drawing.Color.LightGreen;
