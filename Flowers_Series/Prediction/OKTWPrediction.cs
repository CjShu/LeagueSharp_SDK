namespace Flowers_Series.Prediction
{
    ...
    public static class OKTWPrediction
    {
        ...
        public class PredictionInput
        {
            ...
            public Obj_AI_Base Unit = GameObjects.Player;
            public Obj_AI_Base Source = GameObjects.Player;
            public bool UseBoundingRadius = true;

            public Vector3 From
            {
                get { return _from.ToVector2().IsValid() ? _from : GameObjects.Player.ServerPosition; }
                set { _from = value; }
            }
            ...
            public Vector3 RangeCheckFrom
            {
                get
                {
                    return _rangeCheckFrom.ToVector2().IsValid()
                        ? _rangeCheckFrom
                        : (From.ToVector2().IsValid() ? From : GameObjects.Player.ServerPosition);
                }
        ...
        public static class Prediction
        {
            ...
            internal static PredictionOutput WayPointAnalysis(PredictionOutput result, PredictionInput input)
            {
                ...
                if (input.Unit.HealthPercent < 20 || GameObjects.Player.HealthPercent < 20)
                {
                    result.Hitchance = HitChance.VeryHigh;
                    return result;
                }
            ...
            internal static PredictionOutput GetDashingPrediction(PredictionInput input)
            {
                ...
                if (!dashData.IsBlink)
                {
                    ...
                    var dashPred = GetPositionOnPath(
                        input, new List<Vector2> { input.Unit.ServerPosition.ToVector2(), endP }, dashData.Speed);
            ...
            internal static PredictionOutput GetPositionOnPath(PredictionInput input, List<Vector2> path, float speed = -1)
            {
                    ...
                    Math.Abs(input.Speed - float.MaxValue) > float.Epsilon)
                {
                    ...
                    for (var i = 0; i < path.Count - 1; i++)
                    {
                        ...
                        var sol = Common.Geometry.VectorMovementCollision(a, b, speed, input.From.ToVector2(), input.Speed, tT);
                        var t = (float)sol[0];
        ...
        public static class AoePrediction
        {
            ...
            public static class Cone
            {
                    ...
                    var posibleTargets = new List<PossibleTarget>
                {
                    ...
                    if (posibleTargets.Count > 1)
                    {
                        ...
                        if (bestCandidateHits > 1 && input.From.ToVector2().Distance(bestCandidate, true) > 50 * 50)
                        {
                            return new PredictionOutput
                            {
                                Hitchance = mainTargetPrediction.Hitchance,
            ...
            public static class Line
            {
                ...
                internal static Vector2[] GetCandidates(Vector2 from, Vector2 to, float radius, float range)
                {
                    ...
                    var intersections = Common.Geometry.CircleCircleIntersection(
        ...
        public static class Collision
        {
            private static bool MinionIsDead(PredictionInput input, Obj_AI_Base minion, float distance)
            {
                ...
                return Health.GetPrediction(minion, convert, 0) <= 0;
            }
                    ...
                    foreach (var objectType in input.CollisionObjects)
                    {
                        switch (objectType)
                        {
                            case CollisionableObjects.Minions:
                                foreach (var minion in GameObjects.EnemyMinions.Where(x => x.IsValidTarget(Math.Min(input.Range + input.Radius + 100, 2000), true, input.From)))
                                {
                                    ...
                                    if (minionPos.ToVector2().Distance(input.From.ToVector2(), position.ToVector2(), true, true) <= Math.Pow((input.Radius + bonusRadius + minion.BoundingRadius), 2))
                                    {
                                        if (MinionIsDead(input, minion, distanceFromToUnit))
                                            continue;
                                        return true;
                                    ...
                                    )
                                {
                                    ...
                                    if (
                                        prediction.UnitPosition.ToVector2()
                                            .Distance(input.From.ToVector2(), position.ToVector2(), true, true) <=
        ...
        public static class UnitTracker
        {
            ...
            static UnitTracker()
            {
                ...
                {
                    Champion.Add(hero);
                    UnitTrackerInfoList.Add(new UnitTrackerInfo() { NetworkId = hero.NetworkId, AaTick = Variables.TickCount, StopMoveTick = Variables.TickCount, NewPathTick = Variables.TickCount, SpecialSpellFinishTick = Variables.TickCount, LastInvisableTick = Variables.TickCount });
                }
            ...
            {
                var TrackerUnit = UnitTrackerInfoList.Find(x => x.NetworkId == unit.NetworkId);
                var points = new List<Vector2> {unit.ServerPosition.ToVector2()};
                return points;
            }
